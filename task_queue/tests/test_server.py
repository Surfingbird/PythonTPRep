...
class ServerBaseTest(TestCase):
    ...
    def test_order(self):
        """
        Tries to tangle tasks so first task becomes second in queue and vice versa:
        undone: [1,2]  in_progress: []
        undone: [2]  in_progress: [1]
        undone: []  in_progress: [1,2]
        undone: [1]  in_progress: [2]
        undone: []  in_progress: [2,1]
        undone: [2]  in_progress: [1]
        undone: [1,2]  in_progress: [] - DESIRED RESULT
        undone: [2,1]  in_progress: [] - BAD RESULT
        """
        first_task_id = self.send(b'ADD 1 5 12345')
        second_task_id = self.send(b'ADD 1 5 12345')
        self.assertEqual(b'YES', self.send(b'IN 1 ' + first_task_id))
        ...
        # check if first task is expired
        self.assertEqual(b'NO', self.send(b'ACK 1 ' + first_task_id))
        self.assertEqual(first_task_id + b' 5 12345', self.send(b'GET 1'))
        time.sleep(3.1)

        # check if second task is expired
        self.assertEqual(b'NO', self.send(b'ACK 1 ' + second_task_id))
        time.sleep(3.1)
        ...
        # check if first task is first in queue and second is second
        self.assertEqual(first_task_id + b' 5 12345', self.send(b'GET 1'))
        self.assertEqual(second_task_id + b' 5 12345', self.send(b'GET 1'))
    ...
    def test_save_load(self):
        ...
        self.assertEqual(b'SAVED', self.send(b'SAVE'))
        self.tearDown()
        self.setUp()
        self.assertEqual(b'LOADED', self.send(b'LOAD'))
        self.assertEqual(b'YES', self.send(b'IN 1 ' + first_task_id))
        self.assertEqual(b'YES', self.send(b'IN 1 ' + second_task_id))
        self.assertEqual(second_task_id + b' 5 12345', self.send(b'GET 1'))
